...
import { Star, Briefcase, Plus, RotateCcw } from 'lucide-react';
...
export default function BusinessReviewsPage() {
  ...
  const loadReviews = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/reviews?orgId=org-biz-1');
  ...
  const receivedReviews = reviews.filter((r) => r.target_org_id === 'org-biz-1');
  const givenReviews = reviews.filter((r) => r.reviewer_org_id === 'org-biz-1');
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 w-full flex-1">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
              ...
              Vendor Partner Ratings & Reviews
            </h1>
            <p className="text-xs text-slate-500 mt-1">
              Reputation scores and performance feedback from sponsoring Companies and recipient NGOs
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-sm transition"
            >
              <Plus className="h-4 w-4" />
              <span>Review Corporate / NGO</span>
            </button>
          </div>
        </div>
        ...
        <ReviewModal
          ...
          reviewerOrgId="org-biz-1"
          reviewerRole="BUSINESS"
          targetOrgId="org-corp-1"
          targetOrgName="Apex Global Technologies"
